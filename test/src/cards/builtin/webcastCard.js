...
export function createWebcastCard() {
    function renderWebcasts(element, state, sdk) {
        ...
        render: (element, state, sdk) => {
            if (!element._webcastCardInitialized) {
                sdk.updateCard(
                    element,
                    10000,
                    // This is the actual rebuild logic (below), not a stub — it
                    // only re-runs when compareFn detects the webcasts array
                    // itself changed, so a fresh player isn't torn down every
                    // tick just because the clock ticked.
                    renderWebcasts,
            ...
            renderWebcasts(element, state, sdk);
        }
    };
}