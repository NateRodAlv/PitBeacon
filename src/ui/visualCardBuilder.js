...
static get LOGIC_BLOCK_TYPES() {
  return {
    ...
    ON_CLICK: {
      ...
      canHaveChildren: true,
      fields: [
        { id: "elementId", label: "Element ID", type: "text", default: "myButton" },
      ],
      generateJS: (block, childCode) => {
        const elId = block.fields?.elementId || "myButton";
        return `// When clicked\nconst el = document.getElementById('${elId}');\nif (el) {\n  el.addEventListener('click', async function() {\n${childCode}\n  });\n}`;
      },
    },
    ON_CHANGE: {
      id: "on-change",
      label: "✏️ When Value Changes",
      icon: "change",
      category: "events",
      color: "#f39c12",
      shape: "hat",
      canHaveChildren: true,
      fields: [
        { id: "elementId", label: "Element ID", type: "text", default: "myInput" },
      ],
      generateJS: (block, childCode) => {
        const elId = block.fields?.elementId || "myInput";
    ...
    IF: {
      ...
      canHaveChildren: true,
      fields: [
        { id: "condition", label: "Condition (JS expr)", type: "text", default: "true" },
      ],
      generateJS: (block, childCode) => {
    ...
    FOR_EACH: {
      ...
      canHaveChildren: true,
      fields: [
        { id: "listVar", label: "List variable", type: "text", default: "myList" },
        { id: "itemVar", label: "Item variable name", type: "text", default: "item" },
      ],
      generateJS: (block, childCode) => {
        const listVar = block.fields?.listVar || "myList";
        const itemVar = block.fields?.itemVar || "item";
    ...
    FETCH_DATA: {
      ...
      generateJS: (block, childCode) => {
        ...
        const varName = block.fields?.variableName || "myData";
        return `// Fetch data\nsdk.fetchDataSource('${source}').then(data => {\n  vars['${varName}'] = data;\n${childCode}\n}).catch(err => {\n  console.warn('Fetch error:', err);\n});`;
      },
    },
    GET_PROPERTY: {
      id: "get-property",
      ...
      canHaveChildren: false,
      fields: [
        { id: "sourceVar", label: "Source variable", type: "text", default: "myData" },
        { id: "property", label: "Property (e.g. 'length', '0', 'name')", type: "text", default: "length" },
        { id: "targetVar", label: "Store in variable", type: "text", default: "myProperty" },
      ],
      generateJS: (block) => {
        const src = block.fields?.sourceVar || "myData";
        const prop = block.fields?.property || "length";
        const tgt = block.fields?.targetVar || "myProperty";
        // Use bracket notation to handle numbers and strings
    ...
    GET_TEAM_NUMBER: {
      ...
      canHaveChildren: false,
      fields: [
        { id: "targetVar", label: "Store in variable", type: "text", default: "teamNum" },
      ],
      generateJS: (block) => {
        const tgt = block.fields?.targetVar || "teamNum";
    ...
    SET_VARIABLE: {
      ...
      canHaveChildren: false,
      fields: [
        { id: "name", label: "Variable name", type: "text", default: "myVar" },
        { id: "value", label: "Value (JS expr, e.g. 5, {otherVar}, true)", type: "text", default: "0" },
      ],
      generateJS: (block) => {
        const name = block.fields?.name || "myVar";
        const rawValue = block.fields?.value || "0";
        return `vars['${escapeJsString(name)}'] = ${substituteVarsInExpression(rawValue)};`;
      },
    },
    INCREMENT: {
      id: "increment",
      ...
      canHaveChildren: false,
      fields: [
        { id: "name", label: "Variable name", type: "text", default: "counter" },
        { id: "amount", label: "Amount", type: "number", default: 1 },
      ],
      generateJS: (block) => {
        const name = block.fields?.name || "counter";
        const amount = block.fields?.amount || 1;
        return `vars['${name}'] = (vars['${name}'] || 0) + ${amount};`;
      },
    },
    ...
    SET_TEXT: {
      ...
      canHaveChildren: false,
      fields: [
        { id: "elementId", label: "Element ID", type: "text", default: "myText" },
        { id: "text", label: "Text (use {var})", type: "text", default: "Hello!" },
      ],
      generateJS: (block) => {
        const elId = block.fields?.elementId || "myText";
        const text = block.fields?.text || "Hello!";
        return `const el = document.getElementById('${escapeJsString(elId)}');\nif (el) el.textContent = ${interpolateExpr(text)};`;
      },
    },
    SET_VALUE: {
      id: "set-value",
      ...
      fields: [
        { id: "elementId", label: "Element ID", type: "text", default: "myInput" },
        { id: "value", label: "Value (use {var})", type: "text", default: "New value" },
      ],
      generateJS: (block) => {
        const elId = block.fields?.elementId || "myInput";
        const value = block.fields?.value || "New value";
        return `const el = document.getElementById('${escapeJsString(elId)}');\nif (el) el.value = ${interpolateExpr(value)};`;
      },
    },
    SET_STYLE: {
      id: "set-style",
      ...
      canHaveChildren: false,
      fields: [
        { id: "elementId", label: "Element ID", type: "text", default: "myElement" },
        { id: "property", label: "Property", type: "select", default: "color", options: ["color", "background", "fontSize", "display", "opacity"] },
        { id: "value", label: "Value (use {var})", type: "text", default: "red" },
      ],
      generateJS: (block) => {
        const elId = block.fields?.elementId || "myElement";
    ...
    SHOW_HIDE: {
      ...
      fields: [
        { id: "elementId", label: "Element ID", type: "text", default: "myElement" },
        { id: "action", label: "Action", type: "select", default: "show", options: ["show", "hide", "toggle"] },
      ],
      generateJS: (block) => {
        const elId = block.fields?.elementId || "myElement";
    ...
    TRIGGER_ALARM: {
      ...
      canHaveChildren: false,
      fields: [
        { id: "type", label: "Alarm Type", type: "select", default: "custom", options: ["match", "note", "custom"] },
      ],
      generateJS: (block) => {
        const type = block.fields?.type || "custom";
        return `sdk.triggerAlarm('${type}');`;
      },
    },
    NOTIFY: {
      id: "notify",
    ...
    CUSTOM_JS: {
      ...
      canHaveChildren: false,
      fields: [
        { id: "code", label: "JavaScript (one or more lines)", type: "text", default: "// your code here" },
      ],
      generateJS: (block) => {
        const code = block.fields?.code || "";
        // Insert raw code; user can use vars, sdk, interpolate
        return code;
      },
    },
  };
  ...
  _renderLogicBlock(block, index, parentElement) {
    ...
    el.innerHTML = `
            ...
            ${canHaveChildren ? `<div class="vb-logic-children" data-parent-id="${block.id}"><p class="vb-logic-child-hint">Drop blocks here</p></div>` : ""}
        `;
    ...
    el.addEventListener("drop", (e) => {
      ...
      if (movedId) {
        this._moveLogicBlock(movedId, block.id, isBefore ? "before" : "after");
        return;
      }
    ...
    type.fields.forEach((field) => {
      ...
      if (isElementIdField) {
        ...
        if (currentVal && !existingIds.includes(currentVal)) {
          existingIds.unshift(currentVal); // keep a stale/custom value visible & selected
        }
  ...
  _generateCardCode() {
    ...
    this._cardData.elements.forEach((element) => {
      ...
      if (props.styles && Object.keys(props.styles).length > 0) {
        const selector = props.id ? `#${props.id}` : `.element-${element.type}`;
